...
<div class="min-h-screen flex items-center justify-center bg-stone-100 py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
    ...
    <div class="max-w-2xl w-full space-y-8 bg-white/90 backdrop-blur-md p-10 rounded-2xl shadow-2xl relative z-10 border border-white/40">
        <div>
            <h2 class="mt-2 text-center text-3xl font-extrabold text-stone-900 font-playfair"><i class="fas fa-edit text-amber-700 mr-2"></i> Update Global Item</h2>
            <p class="mt-2 text-center text-sm text-stone-600">
                Modify the details below to update the inventory record.
            </p>
        </div>
        ...
        <form class="mt-8 space-y-6" action="${pageContext.request.contextPath}/admin/saveBook" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    <label for="barcode" class="block text-sm font-medium text-stone-700 mb-1">Book Barcode (ID)</label>
                    <input id="barcode" name="barcode" type="text" value="${book.barcode}" readonly class="appearance-none block w-full px-3 py-3 border border-stone-200 rounded-md shadow-sm text-stone-500 sm:text-sm bg-stone-100 cursor-not-allowed">
                </div>

                <div class="md:col-span-2">
                    <label for="name" class="block text-sm font-medium text-stone-700 mb-1">Book Name</label>
                    <input id="name" name="name" type="text" value="${book.name}" required class="appearance-none block w-full px-3 py-3 border border-stone-300 rounded-md shadow-sm placeholder-stone-400 focus:outline-none focus:ring-amber-500 focus:border-amber-500 sm:text-sm bg-white/70">
                </div>
                
                <div class="md:col-span-2">
                    <label for="author" class="block text-sm font-medium text-stone-700 mb-1">Author Name</label>
                    <input id="author" name="author" type="text" value="${book.author}" required class="appearance-none block w-full px-3 py-3 border border-stone-300 rounded-md shadow-sm placeholder-stone-400 focus:outline-none focus:ring-amber-500 focus:border-amber-500 sm:text-sm bg-white/70">
                </div>

                <div>
                    <label for="price" class="block text-sm font-medium text-stone-700 mb-1">Price (₹)</label>
                    <input id="price" name="price" type="number" step="0.01" min="0" value="${book.price}" required class="appearance-none block w-full px-3 py-3 border border-stone-300 rounded-md shadow-sm placeholder-stone-400 focus:outline-none focus:ring-amber-500 focus:border-amber-500 sm:text-sm bg-white/70">
                </div>
                
                <div>
                    <label for="quantity" class="block text-sm font-medium text-stone-700 mb-1">Stock Quantity</label>
                    <input id="quantity" name="quantity" type="number" min="0" value="${book.quantity}" required class="appearance-none block w-full px-3 py-3 border border-stone-300 rounded-md shadow-sm placeholder-stone-400 focus:outline-none focus:ring-amber-500 focus:border-amber-500 sm:text-sm bg-white/70">
                </div>

                <div>
                    <label for="image" class="block text-sm font-medium text-stone-700 mb-1">Update Cover Image</label>
                    ...
                    <span class="text-xs text-stone-500 mt-1 block">Leave empty to keep current</span>
                </div>

                <div>
                    <label for="pdf" class="block text-sm font-medium text-stone-700 mb-1">Update Book PDF</label>
            ...
            <div class="flex items-center justify-between pt-4">
                ...
                <button type="submit" class="inline-flex justify-center py-3 px-6 border border-transparent shadow-lg text-sm font-bold rounded-md text-white bg-amber-800 hover:bg-amber-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500 transition transform hover:-translate-y-0.5">
                    <i class="fas fa-save mr-2 mt-0.5"></i> Update Book
                </button>
            </div>
        </form>
    </div>
